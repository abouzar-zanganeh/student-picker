...
این برنامه اکنون یک اپلیکیشن وب پیش‌رونده (PWA) است، به این معنی که می‌توانید آن را مستقیماً از مرورگر روی تلفن همراه یا کامپیوتر خود نصب کنید.

تجربه کاربری مشابه اپلیکیشن: پس از نصب، برنامه در یک پنجره مستقل و بدون نوار آدرس مرورگر اجرا می‌شود و تجربه‌ای شبیه به یک اپلیکیشن بومی ارائه می‌دهد.

عملکرد آفلاین: به لطف استفاده از Service Worker، تمام فایل‌های اصلی برنامه (مانند HTML, CSS, JavaScript و حتی فونت‌ها) در حافظه دستگاه شما ذخیره (Cache) می‌شوند. این به شما امکان می‌دهد تا حتی در صورت عدم دسترسی به اینترنت، برنامه را باز کرده و از آن استفاده کنید.

دسترسی سریع: با نصب برنامه، آیکون آن به صفحه اصلی (Home Screen) دستگاه شما اضافه می‌شود و دسترسی به آن را سریع و آسان می‌کند.
...
PWA (Manifest, Service Worker) برای قابلیت نصب و عملکرد کاملاً آفلاین با ذخیره‌سازی تمام Asset ها (شامل فونت‌ها) به‌صورت محلی.

📂 نحوه استفاده
این برنامه کاملاً سمت کاربر است و نیازی به نصب ابزار خاصی ندارد:

پروژه را دانلود و از حالت فشرده خارج کنید.
