...
export function showReportToAdminModal(student, session) {
    ...
    // Use the new standalone modal
    showReportModal(
        student,
        session,
        message,
        (finalMessage, contact) => {
            // Callback after report is sent and confirmed
            // The note is already saved by the standalone modal
            // We just need to do any additional cleanup if needed
        }
    );
}
 ...
 */
export function showReportModal(student, session, preFilledMessage, onReportSent) {
